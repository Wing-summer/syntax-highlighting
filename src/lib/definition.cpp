...
QString Definition::translatedName() const
{
    if (d->translatedName.isEmpty()) {
        d->translatedName = QCoreApplication::instance()->translate(
            "Language",
            d->nameUtf8.isEmpty() ? d->name.toUtf8().constData() : d->nameUtf8.constData());
    }
...
QString Definition::translatedSection() const
{
    if (d->translatedSection.isEmpty()) {
        d->translatedSection = QCoreApplication::instance()->translate(
            "Language Section",
...
bool Definition::foldingEnabled() const
{
    ...
    return d->foldingRegionsState == DefinitionData::FoldingRegionsState::ContainsFoldingRegions
           || d->indentationBasedFolding;
}
...
QList<Format> Definition::formats() const
{
    ...
    std::sort(
        formatList.begin(),
        formatList.end(),
        [](const KSyntaxHighlighting::Format &lhs, const KSyntaxHighlighting::Format &rhs) {
    ...
    if (isValid()) {
        ...
        while (!queue.empty()) {
            ...
            for (const auto *defData : def->immediateIncludedDefinitions) {
                ...
                if (std::find_if(definitions.begin(), definitions.end(), pred)
                    == definitions.end()) {
...
bool DefinitionData::loadLanguage(QXmlStreamReader &reader)
{
    ...
    for (auto &n :
#if QT_VERSION < QT_VERSION_CHECK(6, 0, 0)
         names.split(';', Qt::SkipEmptyParts)
#else
         QStringTokenizer(names, u';', Qt::SkipEmptyParts)
#endif
    ...
    for (auto &ext :
#if QT_VERSION < QT_VERSION_CHECK(6, 0, 0)
         exts.split(';', Qt::SkipEmptyParts)
#else
         QStringTokenizer(exts, u';', Qt::SkipEmptyParts)
#endif
    ) {
    ...
    for (auto &mt :
#if QT_VERSION < QT_VERSION_CHECK(6, 0, 0)
         mts.split(';', Qt::SkipEmptyParts)
#else
         QStringTokenizer(mts, u';', Qt::SkipEmptyParts)
#endif
    ) {
        mimetypes.push_back(mt.toString());
    }
    if (reader.attributes().hasAttribute(QLatin1String("casesensitive"))) {
        caseSensitive = Xml::attrToBool(reader.attributes().value(QLatin1String("casesensitive")))
                            ? Qt::CaseSensitive
                            : Qt::CaseInsensitive;
    }
    return true;
    ...
    while (!reader.atEnd()) {
        ...
        case QXmlStreamReader::StartElement:
            ...
            if (reader.name() == QLatin1String("keywords")) {
                if (reader.attributes().hasAttribute(QLatin1String("casesensitive"))) {
                    caseSensitive = Xml::attrToBool(
                                        reader.attributes().value(QLatin1String("casesensitive")))
                                        ? Qt::CaseSensitive
                                        : Qt::CaseInsensitive;
                }

                // adapt wordDelimiters
                wordDelimiters.append(
                    reader.attributes().value(QLatin1String("additionalDeliminator")));
                ...
                if (reader.attributes().hasAttribute(QLatin1String("indentationsensitive"))) {
                    indentationBasedFolding = Xml::attrToBool(
                        reader.attributes().value(QLatin1String("indentationsensitive")));
                }
    ...
    while (!reader.atEnd()) {
        ...
        case QXmlStreamReader::StartElement:
            ...
            if (reader.name() == QLatin1String("comment")) {
                const bool isSingleLine = reader.attributes().value(QLatin1String("name"))
                                          == QLatin1String("singleLine");
                if (isSingleLine) {
                    singleLineCommentMarker
                        = reader.attributes().value(QLatin1String("start")).toString();
                    const bool afterWhiteSpace = reader.attributes().value(
                                                     QLatin1String("position"))
                                                 == QLatin1String("afterwhitespace");
                    singleLineCommentPosition = afterWhiteSpace ? CommentPosition::AfterWhitespace
                                                                : CommentPosition::StartOfLine;
                } else {
                    multiLineCommentStartMarker
                        = reader.attributes().value(QLatin1String("start")).toString();
                    multiLineCommentEndMarker
                        = reader.attributes().value(QLatin1String("end")).toString();
                }
            }
            reader.readNext();
            break;
...
bool DefinitionData::checkKateVersion(QStringView verStr)
{
    ...
    if (idx <= 0) {
        qCWarning(Log) << "Skipping" << fileName
                       << "due to having no valid kateversion attribute:" << verStr;
        return false;
    }
    const auto major = verStr
                           .
#if QT_VERSION < QT_VERSION_CHECK(6, 0, 0)
                       mid
#else
    ...
    if (major > KSYNTAXHIGHLIGHTING_VERSION_MAJOR
        || (major == KSYNTAXHIGHLIGHTING_VERSION_MAJOR
            && minor > KSYNTAXHIGHLIGHTING_VERSION_MINOR)) {
...
DefinitionData::ResolvedContext DefinitionData::resolveIncludedContext(
    QStringView defName, QStringView contextName)
{
    if (defName.isEmpty()) {
    ...
    if (d.isValid()) {
        ...
        if (resolvedDef != this) {
            if (std::find(
                    immediateIncludedDefinitions.begin(),
                    immediateIncludedDefinitions.end(),
                    resolvedDef)
                == immediateIncludedDefinitions.end()) {
                immediateIncludedDefinitions.push_back(resolvedDef);
                resolvedDef->load();
                if (resolvedDef->foldingRegionsState
                    == FoldingRegionsState::ContainsFoldingRegions) {
                    foldingRegionsState = FoldingRegionsState::ContainsFoldingRegions;
                }
            }
        }

...
{
    if (offset + pattern.size() <= text.size() && text.
#if QT_VERSION < QT_VERSION_CHECK(6,0,0)
                                                      mid
#else
                                                      sliced
#endif
                                                      (offset, pattern.size()).compare(pattern, caseSensitivity) == 0) {
...
MatchResult KeywordListRule::doMatch(QStringView text, int offset, const QStringList &, DynamicRegexpCache &) const
{
    ...
    if (m_keywordList.contains(text.
#if QT_VERSION < QT_VERSION_CHECK(6,0,0)
                               mid
#else
                               sliced
#endif
                               (offset, newOffset - offset), m_caseSensitivity)) {
        return newOffset;
    }
...
MatchResult WordDetect::doMatch(QStringView text, int offset, const QStringList &, DynamicRegexpCache &) const
{
    ...
    if (text.
        ...
        sliced
#endif
        (offset, m_word.size()).compare(m_word, m_caseSensitivity) != 0) {
        return offset;
    }
